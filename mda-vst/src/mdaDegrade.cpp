...
void mdaDegrade::degrader::init()
{
    buf0 = buf1 = buf2 = buf3 = buf4 = buf5 = buf6 = buf7 = buf8 = buf9 = 0.0f;
}

void mdaDegrade::degrader::update(float fParam1, float fParam2, float fParam3, float fParam4, float fParam5, float fParam6, float sampleRate)
{
    float f;
    ...
    float hz = (float)pow(10.0f, 2.30104f + 2.f * fParam4);
    float j, k, r = 0.999f;
    j = r * r - 1;
    k = (float)(2.f - 2.f * r * r * cos(0.647f * hz / sampleRate));
    fo2 = (float)((sqrt(k * k - 4.f * j * j) - k) / (2.f * j));
...
void mdaDegrade::degrader::process(float* in, float* out, int sampleFrames)
{
...
mdaDegrade::mdaDegrade(audioMasterCallback audioMaster) : AudioEffectX(audioMaster, 1, 6)	// 1 programs, 6 parameters
{
...
void mdaDegrade::setParameter(VstInt32 index, float value)
{
    ...
    left.update(fParam1, fParam2, fParam3, fParam4, fParam5, fParam6, getSampleRate());
    right.update(fParam1, fParam2, fParam3, fParam4, fParam5, fParam6, getSampleRate());
}
...
void mdaDegrade::getParameterDisplay(VstInt32 index, char* text)
{
    ...
    case 2: int2strng((VstInt32)(getSampleRate() / left.tn), text); break;
...
{
    left.process(inputs[0], outputs[0], int(sampleFrames));
    right.process(inputs[1], outputs[1], int(sampleFrames));
}
